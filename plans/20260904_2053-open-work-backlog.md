...
|---|---|---|---|
| **H1** | Part II's "version-stamped" promise unkept in **6** of 21 docs | medium | Zero `v2.1.x` markers in ~~`03`~~ `05 06 15` ~~`16`~~ `19` ~~`29`~~ `31` `35` — **`16` closed 20260906 by C5, 0 → 8 markers**; `docs/11` has 34. **`03`, `29` and `18` stamped 20260905 (PR #36)**. **Correction, confirmed 20260906: `35` was never actually stamped** — its earlier strikethrough was wrong, and contradicted this row's own "`35` re-check pending" clause; `grep -c 'v2\.1\.[0-9]' docs/35-choosing-your-mode.md` still returns 0, so the real count is **6**, not 5. Promised at `docs/index.md:76-77` and `LOOP_ENGINEERING.md:78` (both still current). Fold the rest into **C7** |
...
> content risk. **H1**'s remainder is `05 06 15 19 31 35` — **`35` was never actually stamped**,
