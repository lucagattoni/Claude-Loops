...
- **H14** — the two live DST-dependent comments (`scripts/com.luca.loop-news.plist:15`,
  ...
  so the 2026-10-25 date is removed rather than deferred to 2027-03-29.
- Backlog corrected where this work proved it wrong: C4's `D1` half was already shipped; H14's
  `CHANGELOG.md:867` is `:1609` and is a historical entry that stays as written; H14's live
...
7. **Backlog line numbers are stale by default.** Both of step 13's items cited lines that had
   drifted (`SKILL.md:300` gone, `CHANGELOG.md:867` → `:1609`). Re-grep the anchor text; never
   edit by the line number a plan records.
