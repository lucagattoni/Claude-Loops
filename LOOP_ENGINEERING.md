...
| # | Topic | Summary |
|---|---|---|
| 1 | [The Paradigm Shift](https://lucagattoni.github.io/Claude-Loops/01-paradigm-shift/) | Old-way prompting vs. loops; compound probability argument (0.9^10 = 35%); the New Software Lifecycle; five Anthropic canonical patterns; domain expertise (not coding background) predicts work delegated per instruction (Anthropic, ~400K sessions, Jun 2026); founding "replace yourself as the prompter" line (Cherny/Osmani) reprised at company-building scale (a16z's Acharya) and as an "Agent Lite" practitioner framing (Cobus Greyling, Sep 2026); Anthropic's startup guide (Sep 2026) — loops as agents repeating cycles to a stop condition, Translucent code reviewer worked example |
...
| 15 | [Explore → Plan → Implement → Commit](https://lucagattoni.github.io/Claude-Loops/15-explore-plan-implement/) | **Plan mode in practice.** The four-phase recipe (explore, plan, implement, commit); `Shift+Tab` / `/plan` / `--permission-mode plan` to enter, `Ctrl+G` to edit the plan before approving, what plan mode does and does not auto-approve; the `Explore` and `Plan` built-in subagent types; when planning is waste |
...
| 8 | [Permissions & Auto Mode](https://lucagattoni.github.io/Claude-Loops/08-permissions/) | Allow/deny/ask lists, auto mode, risk-tiered authorization by consequence, safety path denylist, agent trust ramp (4-stage), Reject+Replan pattern; ASK verdict + soft cost thresholds (omnigent's `cost_budget` policy, not a `.claude/settings.json` key, now extended to orchestrator-set subagent budgets via an approval-card refinement — formerly a silent hard block); evaluation order for a layered gate |
...
| # | Topic | Summary |
|---|---|---|
| 11 | [Cost & Turn Control](https://lucagattoni.github.io/Claude-Loops/11-cost-control/) | **The reach rule — a checker pays off only where it covers how the thing actually fails (boot probe ≈35% of a shell's tokens; full shell 2.35× the no-tool baseline)**; `--max-turns`, `--max-budget-usd`, effort levels; per-turn model/effort routing busts the prompt cache — pick model and effort per session, not per call; token cost by loop pattern (noop 3-5K → action run 200-250K); early exit rule; operational kill/pause/slow-down thresholds; reasoning effort as the dominant reliability lever (first-try-perfect 28%→89% for +9–29%; testing tools add 42–68% cost, no benefit); confidence-scheduled verification (DeepSpark); multi-dimensional budget pressure (single scalar across tokens/tool-calls/wall-time/dollars, degradation thresholds); fleet-scale telemetry (~$600/day median coding-agent spend per researcher, $7K+ p90, 3.1:1 agent-to-human ratio — OpenAI); Ponytail overengineering countermeasure (-54% LOC/-22% tokens/-20% cost, 100% safety held) |
