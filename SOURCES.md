...
| The New Stack | rss | https://thenewstack.io/feed/ | Active loop engineering coverage. **WebFetch on thenewstack.io returns the newsletter-signup wrapper, not the article body — use `curl` + tag-stripping for full-text reads of this source** (confirmed Sep 2026) |
...
| X search — loop engineering | x-search | https://x.com/search?q=loop%20engineering&src=typed_query&f=live | Live keyword search; dynamically loaded — scroll ≥3 times to surface 20+ posts. **`from:<handle>` queries drop their author scope when the OR-clause isn't parenthesized, returning unrelated global results — wrap it: `from:<handle> ("term1" OR "term2" ...)`** (confirmed independently by multiple sweep agents, Sep 2026) |
...
| GitHub search — loop engineering claude | github-search | https://api.github.com/search/repositories?q=%22loop+engineering%22+claude&sort=updated | Search GitHub repos combining "loop engineering" + claude; returns JSON — no auth needed (10 req/hr limit); find new implementations. **179 total results, heavily diluted by same-day 0-star template/farm repos — add a stars floor (e.g. `stars:>3`) to cut noise** (Sep 2026) |
| GitHub search — claude code harness | github-search | https://api.github.com/search/repositories?q=%22claude+code%22+harness&sort=updated | Search GitHub repos combining "claude code" + harness; returns JSON — surface new harness design repos. **4,529 total results as of 2026-09-21, still heavily diluted — narrow to `q="claude code" harness loop` or `q="claude code" harness orchestrat`, and/or add a `stars:>5` floor** |
...
| ruvnet | github | https://github.com/ruvnet | Prolific actor, now 10+ relevant contributions: metaharness, ruflo, dream-machine, autogenous, openAVO, sparc, ruClip, RuVector, rvm, ruos, agentic-flow (multi-agent orchestration, 66 self-learning agents), open-claude-code (clean-room Claude Code CLI reimplementation) — an entire ecosystem of Loop Contract / self-improving-harness / fleet-infrastructure repos; track the profile for new repos, this list is not final (Sep 2026). **2026-09-21: profile "updated" badges on metaharness/sparc/rvm did not correspond to any main-branch commits after 2026-08-27/09-03 when checked directly — don't trust the badge alone, verify against `/commits/main`** |
...
| Hanako | x | https://x.com/hanakoxbt | Published 2+ substantive loop-vs-graph explainer threads/videos this month (Aug 23 full course + Sep 5-6 "Loops vs Graphs" thread) with meaningful engagement (Sep 2026). **2026-09-21: crypto-promotion question settled — resolved, no action needed.** A full profile scroll (13 days, 20+ posts) found zero $GRAPHRUN or other crypto-token promotion; the only crypto-adjacent element is that the linked paid course accepts USDT as a payment rail alongside Stripe, which is not token promotion. **Separate, still-open fact-check flag:** the account's Sep 9–19 post series opens each post with a quote falsely attributed to named public figures (Sam Altman with 4+ mutually exclusive wordings, a fake "Anthropic engineer", Karpathy), all funneling to the same paid course — **never cite these attributed quotes as real; only the original loop/graph conceptual content is usable.** Also flagged 2026-09-21: cross-promoted the same "Jev"/"Jev Engineering" product as @akshay_pachaar in the same window (Sep 15-20) with near-identical framing — treat as a single coordinated push, not two corroborating sources, if either is cited |
...
| Akshay Pachaar | x | @akshay_pachaar | Co-founder @dailydoseofds_; loop engineering internals diagrams and practitioner breakdowns (Jun 2026). **2026-09-21:** cross-promoted the same "Jev"/"Jev Engineering" non-LLM decision-engine product as @hanakoxbt in the same window (Sep 15-20) with near-identical framing — likely a coordinated sponsor push; treat as a single source, not two corroborating ones, if either is cited |
...
| loop-js/loop.js | github | https://github.com/loop-js/loop.js | 137★, not previously tracked. Goal/Round/read-only-Verify agent design with a concrete budget-ledger and crash-recovery mechanic — close match to this KB's Loop Contract (Sep 2026) |
| InjayTseng/loop-engineering-on-product | github | https://github.com/InjayTseng/loop-engineering-on-product | Rare concrete before/after evidence (20 overnight rounds with a value gate vs. 112 iterations without) for why a value gate matters (Sep 2026) |
| huvii174/loop-engineering-plugin | github | https://github.com/huvii174/loop-engineering-plugin | Six loop-engineering patterns with citable numeric thresholds (deterministic trigram-similarity circuit breaker, 95%-confidence design gate, tenth-man critic) — cited in docs/27 (Sep 2026) |
| Kanevry/session-orchestrator | github | https://github.com/Kanevry/session-orchestrator | Already the KB's Reference Implementations pick for orchestrator design (docs/32); actively released (v4.1.0, Sep 2026) with a dedicated Loop Engineering course page — promoted to a tracked row so future releases are swept automatically rather than only surfacing via manual reading-list checks |
| milosCvetkovicDev/claude-code-monorepo | github | https://github.com/milosCvetkovicDev/claude-code-monorepo | Real production config as a worked maker != checker example, with numbers (Sep 2026) |
| andrewcigan/vibe-dev-plugin | github | https://github.com/andrewcigan/vibe-dev-plugin | Model-tiering-per-stage pattern (higher tier owns planning/critique/verification) with a physically write-denied verifier role — cited in docs/24 and docs/32 (Sep 2026) |
| dmlguq456/hearting | github | https://github.com/dmlguq456/hearting | Sealed per-node model tiers + one harness-manifest.json projected across Claude Code/Codex/OpenCode's native surfaces — cited in docs/24 (Sep 2026) |
| firstintent/ccteam | github | https://github.com/firstintent/ccteam | 425★. Cross-vendor, cross-machine agent-team coordination (Claude Code, Codex, Grok, Kimi, DeepSeek Harness, OpenCode, Pi) with a live cost ledger and per-harness quotas — cited in docs/23 (Sep 2026) |
| bluzername/loop-engineering-kit | github | https://github.com/bluzername/loop-engineering-kit | Named slash-command loops operationalizing verification-as-engine + durable-memory ideas; good primary-source citations for Claude Code primitives docs (Sep 2026) |
| sdsrss/loop_eng | github | https://github.com/sdsrss/loop_eng | Concrete Stop-hook implementation of a hard stopping condition with documented guard-degradation behavior — primary source for stopping-condition coverage (Sep 2026) |
| holny/Agent-Harness-Develop-Book | github | https://github.com/holny/Agent-Harness-Develop-Book | Actively-maintained comparative source-code study across 7 agent harnesses, with a dedicated Loop Engineering chapter and an L1/L2/L3 governance pattern (Sep 2026) |
| loopx-project/loopx | github | https://github.com/loopx-project/loopx | 5,916★, provider-neutral control plane implementing the KB's own Loop Contract concept at production scale; distinct from already-tracked huangruiteng/loopx (Sep 2026) |
| navels/neal | github | https://github.com/navels/neal | Cross-vendor planner/coder/reviewer loop with a model-qualification harness producing a dated PASS/FAIL whitelist — concrete technique for maker/checker coverage (Sep 2026) |
| daonhan/ralph | github | https://github.com/daonhan/ralph | Active implementation of the named "Ralph" loop pattern with a documented security trade-off (Docker socket bind-mount) — worth citing in unattended/headless-risk coverage (Sep 2026) |
| jamesst78/ship-loop-codex | github | https://github.com/jamesst78/ship-loop-codex | States "the reviewer must never be the model that wrote the code" as a hard rule with a worked cross-model gate sequence; port of mstfash/skills — track together (Sep 2026) |
| mstfash/skills | github | https://github.com/mstfash/skills | Upstream origin of the ship-loop pattern (see jamesst78/ship-loop-codex): plan-first, phase-gated loop with a definition-of-done gate (Sep 2026) |
| OutlineDriven/outline-driven-development | github | https://github.com/OutlineDriven/outline-driven-development | 613-skill/28-plugin harness-agnostic distribution model — data point on plugin/skill ecosystem scale (Sep 2026) |
| Untrivial-ai/agent-orchestrator | github | https://github.com/Untrivial-ai/agent-orchestrator | 12,238★ desktop product implementing worktree-per-worker orchestration at scale — named large-scale adopter of the worktree pattern (Sep 2026) |
| Ilko Kacharov (kachar.dev) | x | https://kachar.dev/blog (X: @kachar136) | CTO/Co-founder Juma.ai. 4+ posts directly on-topic on loop/harness/verifier engineering (Sep 2026) |
